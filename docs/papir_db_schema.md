...
| `customerorder_item` | Строки заказа (товары, кол-во, цены). `position_ms_id` — UUID позиции МойСклад (ключ дедупликации; один товар может встречаться в двух строках с разной ценой). `sum_without_discount` = gross (qty×price), `sum_row` = после скидки. |
