...
var RelDocsGraph = (function() {
    ...
    var STATUS_COLOR = window._statusColorMap || {};
    var STATUS_LABEL_MAP = window._statusLabelMap || {};
    }());
...
(function() {
    var _statusColors = _statusInlineStyles;
...
/* ══ NEXT ACTION HINT ══ */
var _naBtn = document.getElementById('nextActionBtn');
if (_naBtn) {
    _naBtn.addEventListener('click', function() {
        var action = _naBtn.dataset.nextAction;
        if (!action || _naBtn.classList.contains('next-action-empty')) return;

        if (action === 'ship') {
            // Визначити спосіб доставки
            var dmId = parseInt(document.getElementById('delivery_method_id').value) || 0;
            var dm = null;
            if (window._deliveryMethods) {
                for (var i = 0; i < _deliveryMethods.length; i++) {
                    if (_deliveryMethods[i].id === dmId) { dm = _deliveryMethods[i]; break; }
                }
            }
            if (!dm) { alert('Не вказано спосіб доставки в замовленні'); return; }

            if (dm.code === 'novaposhta') {
                if (typeof NpTtnModal !== 'undefined') NpTtnModal.open(window._orderId);
            } else if (dm.code === 'ukrposhta') {
                alert('Створення ТТН Укрпошти ще в розробці');
            } else {
                // Кур'єр / Самовивіз — відкрити модалку доставки з потрібним методом
                if (typeof DeliveryModal !== 'undefined') DeliveryModal.open(null, dm.id);
            }
            return;
        }

        // Інші сценарійні дії — поки що заглушка
        alert('Дія: ' + _naBtn.textContent.trim() + '\n(Обробник буде додано)');
    });
}
...
var _personContacts = window._personContacts;
