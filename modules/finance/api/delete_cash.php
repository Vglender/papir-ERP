...
if ($id > 0) {
    ...
    $cur = Database::fetchRow('Papir', "SELECT id_ms, direction FROM finance_cash WHERE id = {$id} LIMIT 1");
    $r = Database::query('Papir', "DELETE FROM finance_cash WHERE id = {$id}");
    if (!$r['ok']) {
        echo json_encode(array('ok' => false, 'error' => 'Помилка видалення'));
        exit;
    }
    // Clean up document_link
    $dlType = ($cur['ok'] && $cur['row'] && $cur['row']['direction'] === 'in') ? 'cashin' : 'cashout';
    ...
    if ($cur['ok'] && $cur['row'] && !empty($cur['row']['id_ms'])) {
        $msDel = finance_ms_cash_delete($cur['row']['id_ms'], $cur['row']['direction']);
...
if ($ids !== '') {
    ...
    $curRows = Database::fetchAll('Papir',
        "SELECT id_ms, direction FROM finance_cash WHERE id IN ({$inClause}) AND id_ms IS NOT NULL AND id_ms != ''"
    );

    $r = Database::query('Papir', "DELETE FROM finance_cash WHERE id IN ({$inClause})");
    if (!$r['ok']) {
        echo json_encode(array('ok' => false, 'error' => 'Помилка видалення'));
        exit;
    }
    // Clean up document_link (cashin/cashout)
    Database::query('Papir', "DELETE FROM document_link WHERE from_type IN ('cashin','cashout') AND from_id IN ({$inClause})");

    $msErrors = array();
    if ($curRows['ok'] && !empty($curRows['rows'])) {
        foreach ($curRows['rows'] as $row) {
            $msDel = finance_ms_cash_delete($row['id_ms'], $row['direction']);
            if (!$msDel['ok']) $msErrors[] = $msDel['error'];
        }
    }
