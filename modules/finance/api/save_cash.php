...
if ($cpId > 0) {
    $cpRow = Database::fetchRow('Papir',
        "SELECT id_ms, name FROM counterparty WHERE id = " . $cpId . " LIMIT 1");
    if ($cpRow['ok'] && $cpRow['row']) {
        $agentMs = $cpRow['row']['id_ms'] ? $cpRow['row']['id_ms'] : null;
...
if ($id > 0) {
    ...
    $curRow = Database::fetchRow('Papir',
        "SELECT id_ms, organization_ms, is_posted, agent_ms_type, agent_ms FROM finance_cash WHERE id = {$id} LIMIT 1"
    );
    ...
    $r = Database::update('Papir', 'finance_cash', $data, array('id' => $id));
    if (!$r['ok']) {
        echo json_encode(array('ok' => false, 'error' => 'Помилка збереження'));
        exit;
    }

    $msSync = finance_ms_cash_push($id,
        array(
            'direction'          => $direction,
            'moment'             => $momentDt,
            'sum'                => round($sum, 2),
    ...
    $data['source']    = 'manual';
    $data['is_posted'] = 1;
    $r = Database::insert('Papir', 'finance_cash', $data);
    if (!$r['ok']) {
    ...
    $msSync = finance_ms_cash_push($localId,
        array(
            'direction'          => $direction,
            'moment'             => $momentDt,
            'sum'                => round($sum, 2),
